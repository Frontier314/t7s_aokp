...
namespace android {
    ...
    void BpSecTVOut::setHdmiCableStatus(uint32_t status)
    {
        ...
        //ALOGD("lt::ISecTVOut::setHdmiCableStatus is running");
...
//added yqf test
    void BpSecTVOut::setHdmiFlip(uint32_t flipVal, uint32_t hwcLayer)
    {
        Parcel data, reply;
        data.writeInt32(flipVal);
        data.writeInt32(hwcLayer);
        remote()->transact(SET_HDMI_FLIP, data, &reply);
    }
//added yqf test
  void BpSecTVOut::saveHdmiRotate(uint32_t rotVal)
    {
        Parcel data, reply;
        data.writeInt32(rotVal);
        remote()->transact(SAVE_HDMI_ROTATE, data, &reply);
    }
