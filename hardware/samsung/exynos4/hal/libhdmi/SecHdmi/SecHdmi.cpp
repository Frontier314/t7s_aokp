...
bool SecHdmi::connect(void)
{
	...
	//else ALOGD("lt::%s::connect:sethdmioutputmode(%d) is ok",__func__,mHdmiOutputMode);//lt,debug
...
#if defined(BOARD_USES_EDID)
        // show display..
        //ALOGI("lt::%s::connect:go to SecHdmiV4L2Utils.cpp",__func__);//lt,debug
        display_menu();
#endif
    }
...
{
//ALOGD("lt::SecHdmi::flush running");
#ifdef DEBUG_MSG_ENABLE
...
if(rotVal==270)	
    ...
    if (hwcLayer == 0) { /* Rotate in UI only mode */
        if (rotVal != mUIRotVal) {
            ...
            mHdmiInfoChange = true;
	     mRotateInfoChange=true; //added yqf	
	     //ALOGI("mUIRotVal:%d,rotVal:%d, mRotateInfoChange:%d\n",mUIRotVal, rotVal,mRotateInfoChange);
	}
