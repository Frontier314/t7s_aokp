...
void AudioHardware::loadRILD(void)
{
    ...
    mSecRilLibHandle = dlopen("libmbm-ril.so", RTLD_NOW);

    if (mSecRilLibHandle) {
		//codewalker
        //LOGV("libsecril-client.so is loaded");
        LOGV("libmbm-ril.so is loaded");
        ...
        if (!openClientRILD  || !disconnectRILD   || !closeClientRILD ||
            ...
            LOGE("Can't load all functions from libmbm-ril.so");
...
#ifdef USE_ULP_AUDIO
                    int fd = open(SRP_CTRL, O_RDONLY);
                    int is_srp_running;
                    ioctl(fd, SRP_CTRL_IS_RUNNING, &is_srp_running);
#endif
                    if (mHardware->mode() != AudioSystem::MODE_IN_CALL) {
#ifdef USE_ULP_AUDIO
                        if (mPcm != 0)
                            doStandby_l();
                        else if (is_srp_running && device == AudioSystem::DEVICE_OUT_WIRED_HEADSET)
                            ;
                        else
                            doStandby_l();
